...
class Universe(object):
    ...
    def __init__(self, k, omega_b, omega_cdm, omega_g, omega_L, omega_nu, accuracy=4e-3, stepsize=0.1):
        ...
        self.H_0 = 2.253e-4 # units Mpc^-1
        self.eta_0 = 1.4135e+04
        ...
        self.Csnd_interp = interp1d(np.log10(cs_load[:,0]), np.log10(cs_load[:,1]), kind='linear',
                             bounds_error=False, fill_value='extrapolate')
        self.timestables = np.loadtxt(path+'/precomputed/Times_Tables.dat')
    ...
    def init_conds(self, eta_0, aval):
        rfactor = self.omega_nu / (.75*self.omega_M*self.scale_a(eta_0)+self.omega_R)
    ...
    def solve_system(self):
        ...
        while (self.eta_vector[-1] < self.eta_0):
            ...
            if np.abs(test_epsilon) > self.accuracy:
                ...
                continue
            self.step += 1
            if np.abs(test_epsilon) < 1e-4*self.accuracy:
                self.stepsize *= 1.25
    ...
    def step_solver(self):
        ...
        for i in range(self.TotalVars):
            ...
            #print self.combined_vector[i][-1]
        #exit()
        return
    ...
    def matrix_J(self, z_val):
        ...
        dTa = -10.**self.Xe(np.log10(eta))*(1.-0.24)*2.503e-7*6.65e-29*1e4/a_val**2./3.24078e-25
        CsndB = self.Csnd(eta)
    ...
    def D_scale_a(self, eta):
        argN = np.argmin(np.abs(self.timestables[:,2]-eta))
        der = (self.timestables[argN+1,1] - self.timestables[argN,1])/(self.timestables[argN+1,2] - self.timestables[argN,2])
        return der
    ...
    def epsilon_test(self, a):
        ...
        #print 'Epsilon: ', (phiTerm + denTerm + velTerm)/denom
        return (phiTerm + denTerm + velTerm)/denom


    def save_system(self):
        sve_tab = np.zeros((len(self.eta_vector), self.TotalVars+1))
        sve_tab[:,0] = self.eta_vector
        for i in range(self.TotalVars):
            sve_tab[:,i+1] = self.combined_vector[i]
        np.savetxt(path + '/OutputFiles/FieldEvolution_{:.4e}.dat'.format(self.k), sve_tab, fmt='%.5e', delimiter='    ')
        return
